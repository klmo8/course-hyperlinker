// This script does the heavy-lifting for the extension
// Gets injected onto webpages by contentscript.js

var ext = "?registration/registration/courses/cmpt";

processResponse(ext);
// @input: the postfix of the API url
// @output: none
// @result: gets JSON from SFU API and does initial processing.
function processResponse(urlExtension) {
  ...
  const courses = [];
  const response = fetch(fullUrl).then(resp =>
    resp.json().then(data => {
      data.map(entry =>
        courses.push({ code: `${entry.value}`, title: `${entry.title}` })
      );
      ...
      // Conversion of array to object adapted from: https://stackoverflow.com/questions/42974735/create-object-from-array
      const courseObj = courses.reduce((acc, el) => {
        // Handling for different capitalization patterns of course codes
        acc[
          `cmpt ${el["code"]}`
        ] = `<a href="${destLink}/${el["code"]}.html" title="${el["title"]}"><mark>cmpt ${el["code"]}</mark></a>`;
        acc[
          `Cmpt ${el["code"]}`
        ] = `<a href="${destLink}/${el["code"]}.html" title="${el["title"]}"><mark>Cmpt ${el["code"]}</mark></a>`;
        acc[
